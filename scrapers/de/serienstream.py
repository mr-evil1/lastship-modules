...
def _normalize(s):
    ...
    & und 'and' werden entfernt, damit z.B. 'His & Hers' == 'his-hers'"""
    if not s:
        return ''
    s = html_unescape(s)
    # & (mit oder ohne Leerzeichen) entfernen
    s = re.sub(r'\s*&\s*', ' ', s)
    # 'and' als eigenstaendiges Wort entfernen
...
class source:
    ...
    def run(self, titles, year, season=0, episode=0, imdb='', hostDict=None):
        ...
        try:
            t = list(set([_normalize(i) for i in titles if i]))
            
            if log_utils:
                logger.info('SerienStream - Search: S%02dE%02d | normalized titles: %s' % (season, episode, t))
                ...
                try:
                    ...
                    if links:
                        ...
                        for href, series_title in links:
                            scraped_normalized = _normalize(series_title)
                            if log_utils:
                                logger.info('SerienStream - Comparing scraped="%s" with %s' % (scraped_normalized, t))
                            for clean_title in t:
                                try:
                                    if clean_title and clean_title in scraped_normalized:
